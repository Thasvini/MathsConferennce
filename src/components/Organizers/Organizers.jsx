import React from "react";
import "./Organizers.css";
import Title from "../title/Title";

const Organizers = () => {
  return (
    <>
      <div className="organizer-heading">
        <h1>Organizing Committee</h1>
        <div className="organizer-grid">
          <div className="organizer-row">
            <div className="role">VICE PATRON</div>
            <div className="desc">
              <b>Mr. R. Mohanram</b>, Chairman, Sri Eshwar College of
              Engineering, Coimbatore, India.
            </div>
          </div>
          <div className="organizer-row">
            <div className="role">CHIEF PATRON</div>
            <div className="desc">
              <b>Mr. R. Rajaram</b>, Director, Sri Eshwar College of
              Engineering, Coimbatore, India.
            </div>
          </div>
          <div className="organizer-row">
            <div className="role">PATRON</div>
            <div className="desc">
              <b>Dr. Sudha Mohanram</b>, Principal, Sri Eshwar College of
              Engineering, Coimbatore, India.
            </div>
          </div>
          <div className="organizer-row">
            <div className="role">GENERAL CHAIR</div>
            <div className="desc">
              <b>Dr. G. Karuppusami</b>, Dean-Research & Innovation, Sri Eshwar
              College of Engineering, Coimbatore, India.
            </div>
          </div>
          <div className="organizer-row">
            <div className="role">PROGRAM ADVISOR</div>
            <div className="desc">
              <b>Dr. H. Anandakumar</b>, Professor, Department of Computer Science and Engineering, Sri Eshwar College of
              Engineering, Coimbatore, India.
            </div>
          </div>
          <div className="organizer-row">
            <div className="role">CONFERENCE CONVENOR</div>
            <div className="desc">
              <b>Dr. R. Maheswari</b>, Head of the Department, Department of
              Mathematics, Sri Eshwar College of Engineering, Coimbatore, India.
            </div>
          </div>
          <div className="organizer-row">
            <div className="role">CONFERENCE CHAIR</div>
            <div className="desc">
              <b>Dr. D. Prasantha Bharathi</b>, Assistant Professor(SL. Gr),
              Department of Mathematics, Sri Eshwar College of Engineering,
              Coimbatore, India.
            </div>
          </div>
        </div>
        <h1>Advisory Board Members</h1>
        <div className="organizer-list">
          <h2>International Advisory Board Committee </h2>
          <ul>
            <li>
              <b>Prof. Dr. Dumitru Baleanu,</b> Lebanese American University,
              Beirut, Professor at Institute of Space Sciences, Bucharest
            </li>
            <li>
              <b> Prof. Dr. Victor Leiva,</b> School of Industrial Engineering
              of the Pontificia Universidad Católica de Valparaíso, Chile
            </li>
            <li>
              <b>Prof. Dr. Sulaiman Nasiru,</b> C. K. Tedam University of
              Technology and Applied Sciences
            </li>
            <li>
              <b>Prof. Dr. Anwar Zeb,</b> COMSATS University Islamabad,
              Abbottabad, Pakistan
            </li>
            <li>
              <b> Prof. Dr. Kamal Shah,</b> Prince Sultan University Riyadh,
              Saudi Arabia
            </li>
            <li>
              <b> Prof. Dr. Christophe Chesnau,</b> Université de
              Caen-Normandie, France
            </li>
            <li>
              <b>Prof. Dr. Carlos Martin-Barreiro,</b> Escuela Superior
              Politécnica del Litoral ESPOL (ROR 04qenc566), Ecuador
            </li>
            <li>
              <b>Prof. Dr. Cecilia Castro,</b> Centre of Mathematics, University
              of Minho
            </li>
            <li>
              <b>Prof. Dr. Rosana Rodriguez,</b> Universidad de Santiago de
              Compostela, Spain
            </li>
            <li>
              <b>Dr. Vadivel Rajarathinam,</b> Phuket Rajabhat University,
              Thailand
            </li>
            <li>
              <b>Dr. A. Parivallal,</b> Sungkyunkwan University, Republic of
              Korea
            </li>
            <li>
              <b>Dr. R. Kavikumar,</b> Chungbuk National University, South Korea
            </li>
          </ul>
        </div>
        <div className="organizer-list">
          <h2>Indian Advisory Board Committee </h2>
          <ul>
            <li>
              <strong>Dr. K. Balachandran</strong>, Bharathiar University, India
            </li>
            <li>
              <strong>Dr. R. Sakthivel</strong>, Bharathiar University, India
            </li>
            <li>
              <strong>Dr. P. Balasubramaniam</strong>, The Gandhigram Rural
              Institute-Deemed to be University, India
            </li>
            <li>
              <strong>Dr. Marshal Anthony</strong>, Anna University
              (Coimbatore), India
            </li>
            <li>
              <strong>Dr. A. Anguraj</strong>, PSG College of Arts & Science,
              India
            </li>
            <li>
              <strong>Dr. K. Somasundaram</strong>, Amrita Vishwa Vidyapeetham,
              India
            </li>
            <li>
              <strong>Dr. N. Anbazhagan</strong>, Alagappa University, India
            </li>
            
            <li>
              <strong>Dr. K. Thilagavathi</strong>, Kongunadu Arts and Science
              College, India
            </li>
            <li>
              <strong>Dr. B. Sundara Vadivoo</strong>, Central University of
              Tamil Nadu, Thiruvarur, India
            </li>
            <li>
              <strong>Dr. T. Jayakumar</strong>, Sri Ramakrishna Mission
              Vidyalaya College of Arts and Science, India
            </li>
            <li>
              <strong>Dr. R. Sivaraj</strong>, Dr B R Ambedkar National
              Institute of Technology Jalandhar, India
            </li>
            <li>
              <strong>Dr. Syed Ali</strong>, Thiruvalluvar University, India
            </li>
            <li>
              <strong>Dr. K. Kalaiarasi</strong>, Cauvery College for Women,
              India
            </li>
            <li>
              <strong>Dr. Karthick SA</strong>, SRM Institute of Science and
              Technology, KTR Campus, India
            </li>
          </ul>
        </div>
        <h1>Faculty Coordinators</h1>
        <div className="organizer-list">
          <ul>
            <li>
              <strong>Dr. N. Valliammal</strong>, Professor
            </li>
            <li>
              <strong>Dr. N. Murugavalli</strong>, Associate Professor
            </li>
            <li>
              <strong>Dr. R. Sree Parimala</strong>, Associate Professor
            </li>
            <li>
              <strong>Dr. R. Saranya</strong>, Assistant Professor
            </li>
            <li>
              <strong>Dr. C. Gokila</strong>, Assistant Professor
            </li>
            <li>
              <strong>Dr. B. Vennila</strong>, Assistant Professor
            </li>
            <li>
              <strong>Dr. M. P. Sindhu</strong>, Assistant Professor
            </li>
            <li>
              <strong>Dr. M. Tamilarasi</strong>, Assistant Professor
            </li>
            <li>
              <strong>Dr. S. Varshini</strong>, Assistant Professor
            </li>
            <li>
              <strong>Mrs. A. Revathy</strong>, Assistant Professor (SL.Gr)
            </li>
            <li>
              <strong>Mrs. V. Deepa</strong>, Assistant Professor (SL.Gr)
            </li>
            <li>
              <strong>Mrs. K. Sivaranjani</strong>, Assistant Professor (SL.Gr)
            </li>
            <li>
              <strong>Mrs. M. M. Shalini</strong>, Assistant Professor (SL.Gr)
            </li>
            <li>
              <strong>Mrs. N. Akiladevi</strong>, Assistant Professor (SL.Gr)
            </li>
            <li>
              <strong>Mrs. R. Hemalatha</strong>, Assistant Professor (SL.Gr)
            </li>
          </ul>
        </div>
      </div>
    </>
  );
};

export default Organizers;
